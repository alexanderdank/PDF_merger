...
server <- function(input, output) {
  ...
  ## reactive extracting the paths of the uploaded files needed for qpdf
  rval_pdf_inputs <- reactive({
    datapaths <- c()
      for (i in input$file$datapath) {
        data <- subset(input$file, datapath == i)
        datapaths <- append(datapaths, data$datapath)
      }
    datapaths
  })
  ...
  ## download button for merged pdfs
   output$download <- downloadHandler(
     ...
     content = function(file) {
       ...
       qpdf::pdf_combine(input = rval_pdf_inputs(), file)
     })
